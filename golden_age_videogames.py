...
get_ipython().run_cell_magic('sql', '', 'SELECT COUNT(g.game)\nFROM game_sales g\nLEFT JOIN reviews r\nON g.game = r.game\nWHERE (critic_score IS NULL AND user_score IS NULL)')
...
get_ipython().run_cell_magic('sql', '', 'SELECT g.year, ROUND(AVG(r.critic_score),2) as avg_critic_score\nFROM game_sales g\nLEFT JOIN reviews r\nON g.game = r.game\nGROUP BY year\nORDER BY 2 DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT g.year,COUNT(g.game) as num_games,ROUND(AVG(r.critic_score),2) as avg_critic_score\nFROM game_sales g\nINNER JOIN reviews r\nON g.game = r.game\nGROUP BY year\nHAVING COUNT(g.game) > 4\nORDER BY 3 DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT g.year,COUNT(g.game) as num_games,ROUND(AVG(r.user_score),2) as avg_user_score\nFROM game_sales g\nINNER JOIN reviews r\nON g.game = r.game\nGROUP BY year\nHAVING COUNT(g.game) > 4\nORDER BY 3 DESC\nLIMIT 10;')
...
get_ipython().run_cell_magic('sql', '', 'SELECT c.year\nFROM top_critic_years_more_than_four_games c\nWHERE c.year in (\n            SELECT u.year\n            FROM top_user_years_more_than_four_games u\n            INNER JOIN top_critic_years_more_than_four_games c\n            ON c.year = u.year\n)')
